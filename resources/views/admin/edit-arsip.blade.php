...
<main class="p-4 sm:ml-64 bg-gray-50 font-roboto min-h-screen">
    <div class="p-0 sm:p-4 mt-14">
        ...
        <section class="mt-4 flex flex-col-reverse md:flex-row gap-2">
            ...
            <form action="{{ route('arsip.update', $arsip->id) }}"  method="POST" id="editArsipForm" enctype="multipart/form-data">
                    @csrf
                    @method('PUT') 
                    ...
                    <div class="w-full mb-4">
                        ...
                        <input type="text" name="nama" id="judul" 
                            ...
                            placeholder="Masukan nama arsip" required autocomplete="off"
                            value="{{ $arsip->nama }}"> <!-- Add value attribute -->
                    ...
                    <div class="w-full mb-4">
                        ...
                        <textarea id="deskripsi" name="deskripsi" rows="4"
                            ...
                            placeholder="Tulis deskripsi arsip">{{ $arsip->deskripsi }}</textarea> 
                        <!-- <p class="mt-2 ml-1 text-sm text-red-600 font-medium flex items-center gap-2">
                            <svg class="w-4 h-4 text-red-600 dark:text-white" aria-hidden="true"
                                xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="w-full" id="fileUploadContainer">
    <label class="block mb-2 font-medium text-gray-900" for="pdf">
        Upload File <span class="text-red-600">*</span>
    </label>
    <div class="flex items-center justify-center w-full relative">
        <label for="pdf" class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded cursor-pointer bg-gray-50 hover:bg-gray-100">
            ...
            <input id="pdf" type="file" class="w-full h-full opacity-0 absolute top-0 left-0 cursor-pointer" name="pdf" />
        </label>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const editArsipForm = document.getElementById('editArsipForm');
        const simpanButton = document.getElementById('simpanButton');

        editArsipForm.addEventListener('submit', function (event) {
