    ...
    <div class="w-full h-screen bg-gray-900 bg-opacity-75 flex items-center ">
        <div class="w-11/12 sm:w-4/5 mx-auto">
            <div class="py-8 px-4 mx-auto max-w-screen-xl text-center lg:py-16">
                <h1 class="mb-4 mt-20 text-3xl sm:text-4xl font-extrabold tracking-tight leading-tight text-white md:text-5xl lg:text-6xl">
                ...
                <div class="w-full md:w-4/5 mx-auto">
                    ...
                    <div class="relative">
                        ...
                        <input type="search" id="search" class="block w-full p-4 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500" placeholder="Cari judul pengumuman...">
                    </div>
                </div>
            </div>
        </div>
...
<section class="w-full md:w-4/5 px-3 md:px-0 pt-8 mx-auto max-w-screen-xl">
    <div class="flex justify-between items-end">
        ...
        <a href="{{ url('/') }}?show_all=true" class="px-3 py-2 text-sm font-medium text-center text-white bg-blue-600 rounded-md hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300">
            Lihat semua
        </a>
...
<script>
    $(document).ready(function(){
        // Berikan CSRF token pada setiap permintaan AJAX
        $.ajaxSetup({
            headers: {
        ...
        {
            $.ajax({
                url: "{{ route('post.liveSearch') }}",
                method: 'GET',
                data: {query: query},
                dataType: 'json',
                success:function(data)
                {
                    // Update elemen HTML dengan hasil pencarian
                    $('.grid').html(data.result_html);
